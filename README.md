...
- [ ] Migrate `ScriptProcessorNode` → `AudioWorkletNode`
- [ ] Speaker diarisation (clinician vs. patient labels)
- [ ] Session persistence (Firestore — `firebase-tools` is already a devDep)
- [ ] ICD-10 / CPT code suggestions inside `assessment` / `plan`
- [ ] Voice commands ("add to plan", "remove that")
- [ ] Export the finalized note to PDF / FHIR
